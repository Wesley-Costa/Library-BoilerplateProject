...
import { AuthorDetailControllerContext } from './authorDetailContoller';
import { AuthorModuleContext } from '../../authorContainer';
import AuthorDetailStyles from './authorDetailStyles';
...
const AuthorDetailView = () => {
	const controller = useContext(AuthorDetailControllerContext);
	const { state } = useContext(AuthorModuleContext);
	const isView = state === 'view';
	const isEdit = state === 'edit';
	const isDetail = state === 'create';
	const { Container, Body, Header, Footer, FormColumn } = AuthorDetailStyles;
		...
		<Container>
			<Header>
				...
				<Typography variant="h5" sx={{ flexGrow: 1 }}>
					{isDetail ? 'Adicionar Item' : isEdit ? 'Editar Item' : controller.document.title}
				</Typography>
				<IconButton
